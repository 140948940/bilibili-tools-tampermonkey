...
import { dynamicAll } from '@/apis'
import { delayToNextMoment, isTimestampToday } from '@/utils/luxon/index'
...
class DynamicVideos extends BaseModule {
   ...
   */
  private async getDynamicVideos() {
    try {
      const response = dynamicAll({ type: 'video' })
